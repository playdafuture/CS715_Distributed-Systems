...
public class Classroom {
     ...
     */
    public Classroom(int capacity, int ns, long st) {
    ...
    void letInStudents(int examNumber) {
        ...
        if (timePassed() < time) {
            Timer t = new Timer("Door timer", startTime, bell, false, time, false);
            t.start();
            synchronized (bell) {
                try {
                    bell.wait();
    ...
    void giveExam(int examNumber) {
        ...
        Timer t = new Timer("Exam timer", startTime, examBell, true, 6000, true);
        t.start();
        synchronized(examBell) {
            try {
                examBell.wait();
